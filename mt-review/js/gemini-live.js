...
const GeminiLive = (() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      _ws.onmessage = (ev) => {
        ...
        for (const p of parts) {
          if (p.inlineData?.data) _playChunk(p.inlineData.data);
        }
      ...
      _ws.onerror = () => { clearTimeout(timer); reject(new Error('WebSocket error')); };
      _ws.onclose = () => {};
    });
  }
  ...
  async function startMic() {
    ...
    _proc    = _micCtx.createScriptProcessor(2048, 1, 1);
    ...
    src.connect(_proc);
    _proc.connect(_micCtx.destination);
  }
  ...
  return { connect, startMic, sendText, disconnect };
})();
